...
Current store is already well-structured. Only change needed: the `Options Store` syntax is valid and does not need migration unless the store is being substantially reworked.
...
Replace inline `const meals = ref([...])` in `MenuPage.vue` with:
...
const { meals, loading } = storeToRefs(mealsStore)  // storeToRefs preserves reactivity
const { fetchMeals } = mealsStore                    // actions don't need storeToRefs
...
router.post('/', authenticate, async (req, res, next) => {
  try {
    const { items } = req.body  // [{ mealId, servings }]
...
**Stripe principle:** Webhook signatures must be verified before any order state change. Use `stripe.webhooks.constructEvent` — never trust payload alone.
...
- [ ] **Step 2: Set Railway environment variables**
  ...
  CLIENT_URL=https://feastbox.vercel.app
  ```
...
- [ ] **Step 4: Add `VITE_API_URL` to Vercel environment**
  ```
...
Expected: Frontend loads, API calls reach Railway backend, meals display from database.

- [ ] **Step 6: Commit**

```bash
git commit --allow-empty -m "chore: Phase 2 deployed — Railway backend + Vercel frontend connected"
```
...
services:
  ...
  # redis: optional future — uncomment when caching is needed
  # redis:
...
- [ ] **Step 4: Verify local stack starts**

Run: `docker compose up --build`
Expected: `api` container starts, `db` starts, API responds at `http://localhost:3000/meals`.
...
GitHub repo → Settings → Secrets → `RAILWAY_TOKEN` = Railway project token.

- [ ] **Step 3: Push to main and verify pipeline passes**

Expected: `test` job runs migrations against test DB and passes. `deploy` job triggers Railway redeploy.

- [ ] **Step 4: Commit**

```bash
git add .github/
git commit -m "feat: GitHub Actions CI/CD — migrate + deploy on push to main"
```
...
These items are not scheduled. Add to a future plan if the project scope expands.

| Feature | Notes |
|---|---|
| Redis caching | Add as a `redis` service in `docker-compose.yml` (already commented in); use for meal list caching. Defer until API response times become a visible problem. |
| OAuth (Google) | `passport-google-oauth20` — adds social login alongside email/password |
| AI meal recommender | Anthropic API — personalised suggestions based on user order history |
| Stripe subscription billing | Only meaningful once a real recurring delivery model is needed |
...
| 3 | "Flatten nested Pinia state" was ambiguous | Clarified: top-level domain grouping is correct, no change needed |
| 4 | No `src/services/api.js` or `src/stores/meals.js` defined for Phase 2 | Both added as explicit tasks with full code |
| 5 | `storeToRefs` not mentioned | Added to Task 10 with explanation of why actions don't need it |
| 6 | Redis in Week 1 schedule | Removed from schedule, moved to Optional/Future with commented docker-compose entry |
| 7 | Week 0 header said "trie-indexed lookup" | Corrected to "debounced computed + multi-field filter" throughout |
| 8 | `vite.config.js` base was `/FeastBox_vue/` for GitHub Pages | Task 1 explicitly changes to `/` for Vercel |
| 9 | GitHub Actions listed as optional Week 2 extra | Promoted to core Phase 3 task |
| 10 | Stripe subscriptions + WebSocket in Week 2 | Moved to Optional/Future — hollow features with no real data |
